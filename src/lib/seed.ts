...
export async function seedDatabase(firestore: Firestore) {
    ...
    const existingLocationsQuery = collection(firestore, "locations");
    const existingLocationsSnap = await collection(firestore, "locations").get();
    existingLocationsSnap.forEach(doc => {
        batch.delete(doc.ref);
    });
    console.log(`Deleting ${existingLocationsSnap.size} old locations.`);
    ...
    for (const location of locations) {
        ...
        for (let i = 1; i <= numRooms; i++) {
            ...
            const room: Omit<Room, 'id'> = {
                ...
                name: `Kamer ${100 + i}`,
                capacity: 4,
                description: `Een standaard kamer met basisvoorzieningen.`
            };
